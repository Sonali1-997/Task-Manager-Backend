...
@Entity('tms_user')
export class User {
  @PrimaryGeneratedColumn({ type: 'bigint', name: 'UserNo' })
  userNo: string;

  @Column({ type: 'nvarchar', length: 100, name: 'Name' })
  name: string;

  @Index('UQ_user_email', { unique: true })
  @Column({ type: 'nvarchar', length: 190, name: 'Email' })
  email: string;

  @Column({ type: 'nvarchar', length: 255, name: 'Password' })
  password: string;
